import json
from math import sqrt, acos, atan, degrees, radians, sin, asin, cos
...
--------------------------------------------------------- \n"""
        ...
        for motor in self.motors:
            print("  {}    {:4} {:4}  {:.4f}".format(motor["motor"], motor["zero"], motor["ninety"], motor["multiplier"]))
    ...
    def plot_file(self, filename="", wait=0.1, interpolate=10, rotate=False, bounds=None):
    ...
    def plot_lines(self, lines=[], wait=0.1, interpolate=10, rotate=False, bounds=None):
