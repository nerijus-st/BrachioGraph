...
def vectorise(
    ...
    if draw_contours and repeat_contours:
        contours = sortlines(
            getcontours(image.resize((int(resolution / draw_contours), int(resolution / draw_contours * h / w))), draw_contours)
        )
    ...
    for x0 in range(w):
        ...
        for y0 in range(h):
            ...
            # below 16, draw diagonal lines and a second horizontal line
                ...
                lg1.append(
                    [(x, y + draw_hatch / 2 + draw_hatch / 4), (x + draw_hatch, y + draw_hatch / 2 + draw_hatch / 4)]
